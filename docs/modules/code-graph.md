...
**Purpose:** Prove Axon (or already-present `codebase-memory-mcp` equivalent) on the host and hand the live path into FreeToken-first Attach OpenCode | Hermes | Grok | Codex. Cite #215.
...
- Failures: neither Axon nor MCP → `FAIL` + `pip install axoniq · ./pfy catalog ask axon`. Hermes/Codex with MCP-only → FAIL + install Axon / Attach grok or opencode.
...
- Callers: `scripts/.pfy` payload (`cmd_code_graph`, `apply_attach_mode_env`); `pfy_attach_mode_208.py` prepare; board `start_sidecar`; Attach 162/196/202/220 via 208 child env.
