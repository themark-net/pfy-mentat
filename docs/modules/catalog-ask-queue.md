...
- What: Tools lists a usable catalog subset (name, category, github, stage, notes, status) — not a scores-only dump. **Ask TUI implement** writes a paste/auto-handoff prompt. **Queue for org** is the live path in [live-org-queue.md](live-org-queue.md) (#214).
- How: [docs/ops/catalog-ask-queue.md](../ops/catalog-ask-queue.md)
- Failures: no attached harness → FAIL + Attach grok|opencode|hermes|codex|claude. HOLD 70–75 or incomplete → SKIP (not auto-lifted). One integration at a time.
