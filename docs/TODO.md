...
| T-0120 | — (ADR-0017; branch `cursor/toolset-harness-handoff-055e`) | P0 doing |
| T-0121 · T-0122 · T-0123 | — (ADR-0017 follow-ups) | P1 |
...
**Session (2026-09-20):** critical review + de-shard — [critical-review-2026-09-20.md](ops/critical-review-2026-09-20.md). **OQ-0011 answered → [ADR-0017](adr/0017-product-catalog-evaluation-handoff-harness.md)** (product = catalog + evaluation + local handoff harness; toolset × harness; hedge). OQ-0013 layout forced by ADR-0017 (`pfylib/`). **Still pending owner: OQ-0012 (feature freeze), OQ-0014 (sharding constraint).**

### Active — design / coding + local path (agent may pick freely)

| ID | Priority | Status | Item | Open questions | Depends | Notes |
|----|----------|--------|------|----------------|---------|-------|
| T-0120 | P0 | doing | **Toolset × harness matrix + Jev reference implementation** (ADR-0017 / G9): `data/toolsets.json` (6 toolsets seeded from #230/#228/#205/#215/#213/#209 with honest per-harness cells), `pfylib/` (`registry`, `toolsets.matrix/plan/apply`, `hedge`, `cli`), `./pfy toolset` + `./pfy hedge` verbs, `tests/` in G0. Jev `implemented` for grok / opencode / claude-code / codex, `partial` hermes, `stub` gemini / exo / continue | — | — | [ADR-0017](adr/0017-product-catalog-evaluation-handoff-harness.md) · [modules/pfylib.md](modules/pfylib.md) · **+2 top-level verbs vs T-0090 — tension owned by OQ-0012** |
| T-0121 | P1 | todo | **Migrate remaining `attach_usable_*` / `pfy_*_NNN.py` clones onto `pfylib/`**: one attach implementation parameterised by `data/harnesses.json`; gab / opencontext / code-graph / orchestration / catalog-ask `plan()` bodies move from stub-with-next-step to real by importing the existing modules; `--selftest`s become `tests/` | OQ-0013 (veto window) | T-0120 | ADR-0017 § Consequences · review §3.2 · keep `./pfy help/status/harness list` byte-identical |
| T-0122 | P1 | todo | **Hedge ledger live spend tracking**: record real cloud spend from harness runs (Gab / TypeSafe / Grok-sub where exposed) into `$PFY_STATE_DIR/hedge-ledger.json`; today `record()` is manual/CLI only; add per-lane receipts to the eval lane | — | T-0120 | `pfylib/hedge.py` · units = integer credits (`PFY_CLOUD_BUDGET`) |
| T-0123 | P1 | todo | **Catalog: lift HOLD, add `implementation` to every row that has one, resume receipts**: decide entries 070–075 on merits (PR #75 Axon merge-or-close), add `implementation` to `TOOLS.md`/`tools.json` rows for Grok CLI bootstrap (harness), codebase-memory-mcp (done in T-0120), agent-loops port, OpenContext / Jev / Gab rows if they clear Stage 0; resume `sources/x-posts.md` receipts | — | T-0120 | ADR-0017 (6) · ADR-0015 slim subset · `scripts/catalog_check.py` cross-checks toolset ids |
| T-0111 | P0 | doing | **De-shard + consolidate operator source**: (a) plain-source entry points + G0 payload gate — **done**; (b) package layout — **`pfylib/` created by T-0120**; (c) migrate clones — T-0121 | OQ-0014 | T-0090 | [critical-review-2026-09-20.md](ops/critical-review-2026-09-20.md) §3, §6 · gate: `scripts/check_no_encoded_payloads.py` |
| T-0090 | P0 | blocked | **Minimal product levers audit**: collapse end-user surface to onboard / stage / ship (≤3 levers **measured on `./pfy help`**, not only `make`) | OQ-0012 | — | [#1](https://github.com/themark-net/pfy-mentat/issues/1) **CLOSED COMPLETED 2026-07-30** (Make ≤5 targets) — `./pfy` now 23 top-level verbs (ADR-0017 added `toolset`, `hedge`); candidates for collapse: per-toolset verbs `decision` / `code-graph` / `context` / `catalog ask` that `toolset plan/apply` generalises · [product-operator-surface.md](ops/product-operator-surface.md) |
...
| — | P1 | done | Live org queue from Tools: real GitHub issue + open/closed/PR status | [#214](https://github.com/themark-net/pfy-mentat/issues/214) · catalog HOLD 70–75 · #198 parked · #76 CLOSED |
...
| — | P1 | done | Operator launch wizard on Loop: compose then Launch session (HTML+tk) | [#225](https://github.com/themark-net/pfy-mentat/issues/225) · catalog HOLD 70–75 · #198 parked · do not reopen #76 |
| — | P1 | done | Session compose: honest lane + enabled toolsets + in-TUI how-to brief | [#224](https://github.com/themark-net/pfy-mentat/issues/224) · catalog HOLD 70–75 · #198 parked · do not reopen #76 |
