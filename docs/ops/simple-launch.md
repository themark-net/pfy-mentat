...
**Attach mode (#208):** operator selects `bare` | `orchestration` | `code-graph` on Attach (HTML+tk). Loop and Attach paint `using: <mode>`. Attach injects skills/AGENTS/prompts/env for that mode (operate-or-FAIL). Unwired mode is FAIL + next — never a silent bare session claiming orchestration or code-graph. One attach / one mode. No Env nav tab. Runbook: [attach-mode.md](attach-mode.md).
