...
When the detector selects FreeToken (or `PFY_LOCAL_RUNTIME=freetoken`), `./pfy start` / `./pfy up` start `ft serve --model …` in the background if `:1919` is not already healthy (model from `PFY_FT_MODEL`, then `LOCAL_CODER_MODEL` / `FREETOKEN_MODEL`), then wait for `GET /v1/models` or `/health`. Missing binary or missing model env is an honest skip (one-liner), not fake ready. Still down after wait = partial.
