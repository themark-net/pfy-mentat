...
`claude-code` (`claude` alias) is a live adapter when `claude` is on PATH. Attach Claude (HTML+tk) and `./pfy start claude` / `claude-code` require a FreeToken-first live endpoint, inherit `LOCAL_OPENAI_BASE_URL` / `OPENAI_BASE_URL`, and prove models list + one smoke before painting attached or exec (#221). No engine or prove fail: FAIL + next (`./pfy up`); inspect equivalent `./pfy models`. Missing binary: FAIL + installer one-liner (`npm install -g @anthropic-ai/claude-code`). Login/credentials/2FA are owner-only and never painted. Optional skills-external path is documentation only.
