...
|----|------|---------------------|--------------------|
| freetoken | inference | Same (port **:1919**). **ready** only when `/v1/models` or `/health` responds; binary + API down = **partial**; no binary = **missing** | Detector-selected or `PFY_LOCAL_RUNTIME=freetoken`: start/up backgrounds `ft serve --model $model` (log under `$PFY_STATE_DIR`) if `:1919` is down, then wait for `/v1/models` or `/health` (12×0.5s). Model env: `PFY_FT_MODEL` then `LOCAL_CODER_MODEL` then `FREETOKEN_MODEL`. Missing binary or no model env: honest skip + `ft serve --model $PFY_FT_MODEL` one-liner, not fake ready. After start, re-detect; still down = partial. Named start exit 2 unless detector ready. Do not vendor or invent `ft` flags. |
