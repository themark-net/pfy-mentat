...
Native operator window **is** the main interface. Tauri 2 when `gui/operator/src-tauri/target/{release,debug}/pfy-operator` exists; else pywebview (`scripts/pfy-gui.py`) with the same frontend (`gui/operator/frontend/`); else stdlib tkinter with live chips from `./pfy status`. Independent poller of detector JSON (`bash scripts/detect-local-runtime.sh --json`), `./pfy status` stdout, and the process table. **No pfy daemon** beyond the native window. Optional `./pfy board --open` is a browser hatch (honest skip if it fails), not the main path. In-window attach for grok/opencode = sidecar subprocess, not exec into the GUI. Board is not a supervisor for arbitrary harnesses. No SaaS, no git console, no credential capture. Do not invent `pfy status --json` unless a real flag exists. Window always opens — missing pywebview is not install-tips-as-UI.
