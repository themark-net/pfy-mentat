...
- **Notes**: Strong fit for local-first agent discipline and OpenCode/Ollama path. Stage 0 pass: Docker Compose self-host documented; Apache 2.0 (engine/agent) + FSL-1.1-ALv2 gateway (→ Apache 2029); clear workflow JSON + plugin quickstarts. opencode support still alpha. Not a primary runtime — pattern + optional guard layer over Grok CLI / OpenCode / agent-cage. Deeper local smoke (Docker + Ollama + one workflow) queued optionally.
