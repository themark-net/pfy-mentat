...
async function attach(id){
  ...
  try{
    ...
    const reach=(j && (j.session_reach||'')) ? String(j.session_reach).trim() : '';
    // #193/#171: never paint attached/READY when ok=false or session_reach FAIL
    const reachFail = (id==='opencode' && (!reach || reach==='FAIL'));
    if(j && j.ok && !reachFail){
      const kind=(j.role==='monitor')?'monitor':id;
      let msg='attached '+kind+(j.pid?(' pid '+j.pid):'')+(reach?(' · '+reach):'');
      if(j.smoke) msg+=' · smoke: '+j.smoke;
      if(j.models && j.models.length) msg+=' · models: '+j.models.length;
      attachMsg=msg;
      attachKind='ok';
      ...
      if(reachFail && detail.indexOf('session_reach')<0 && detail.indexOf('FAIL')!==0)
        detail='session_reach FAIL · '+detail;
      if(detail.indexOf(nxt)<0) detail=detail+' · '+nxt;
      attachMsg='FAIL Attach '+id+' — '+detail;
      attachKind='fail';
      if(id==='opencode') paintSessionReach(reach||'FAIL');
      paintAttach();
