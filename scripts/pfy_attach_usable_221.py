#!/usr/bin/env python3
"""Attach Claude developer-usable session -- cite #221.

Mirrors Codex attach-usable (#220) / Grok (#202) / Hermes (#196) / OpenCode (#193):
FreeToken-first re-probe, child LOCAL_OPENAI_BASE_URL / OPENAI_BASE_URL,
models list + one smoke before ok:True / READY paint. Failures honest
FAIL+next, usable:false. Missing Claude is FAIL+next, never a silent stub.
...
NEXT_INSTALL = "npm install -g @anthropic-ai/claude-code"
...
def _load_162_a():
    ...
    spec = importlib.util.spec_from_file_location("pfy_enterable_162_a_221", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod
...
def _apply_opencontext_env(env):
    """Handoff OpenContext store env into Attach Claude child. Cite #205."""
    path = Path(__file__).resolve().parent / "pfy_opencontext_205.py"
    if not path.is_file():
        return env
    spec = importlib.util.spec_from_file_location("pfy_opencontext_205_221", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
...
def _apply_attach_mode_env(env, STATE=None):
    """Handoff attach mode into Attach Claude child. Cite #208."""
    path = Path(__file__).resolve().parent / "pfy_attach_mode_208.py"
    if not path.is_file():
        return env
    spec = importlib.util.spec_from_file_location("pfy_attach_mode_208_221", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
...
def _claude_bin(which_bin):
    found = which_bin("claude") if callable(which_bin) else ""
    if found:
        return found
    return ""
...
def live_session_reach(STATE, pid_alive=None) -> str:
    """Return Claude session reach only if enterable pid is still alive. Cite #221."""
...
def open_enterable_claude_session(
    *,
    ...
    """Attach Claude + open/focus enterable terminal. Cite #221.
    ...
    against the live detect base AND models list + one smoke succeed (#221).
    Child env inherits LOCAL_OPENAI_BASE_URL / OPENAI_BASE_URL.
    Missing Claude is FAIL+next (never a silent stub success).
    """
    ROOT = Path(ROOT)
    STATE = Path(STATE)
    hid = "claude"
    stub = stub_line or "./pfy start claude"
    bin_path = _claude_bin(which_bin)
    if not bin_path:
        clear_session_reach(STATE)
        return {
            "ok": False,
    ...
    if real and pid_alive(real) and prev_base == base:
        ...
        if active_harness_setter:
            active_harness_setter("claude")
        record_last_verb("start claude")
        return {
            "ok": True,
            "id": hid,
            "live": "READY",
            ...
            "copy": "attached claude pid %s \u00b7 %s \u00b7 %s" % (real, SESSION_REACH_OK, engine),
            "error": "",
            "base_url": base,
            "engine": engine,
            "model": model,
    ...
    if real and pid_alive(real):
        ...
        focused = _focus_pid(real)
        record_sidecar_pid("claude", real)
        write_terminal_pid(STATE, real)
        write_session_reach(STATE, SESSION_REACH_OK)
        if active_harness_setter:
            active_harness_setter("claude")
            ...
            try:
                (STATE / "active-harness").write_text("claude\n", encoding="utf-8")
            except OSError:
                pass
        record_last_verb("start claude")
        try:
...
def prove_cli(base):
    """CLI prove: models list + one smoke. Exit 0 only when usable. Cite #221."""
    next_step = "Launch env or ./pfy up"
...
def cmd_selftest():
    """Missing Claude and missing engine are FAIL+next, never ok:True. Cite #221."""
    import tempfile
    ...
    with tempfile.TemporaryDirectory(prefix="pfy-221-") as tmp:
        state = Path(tmp)
        missing = open_enterable_claude_session(
            ROOT=root,
            STATE=state,
            which_bin=lambda *a: "",
        ...
        check((missing.get("error") or "") == "claude missing", "missing error")
        check(NEXT_INSTALL in (missing.get("next_step") or ""), "missing next install")
        check(missing.get("live") == "FAIL", "missing live FAIL")

        noeng = open_enterable_claude_session(
            ROOT=root,
            STATE=state,
            which_bin=lambda *a: "/tmp/fake-claude-221",
            live_openai_base=lambda: ("", {"engine": "none", "status": "missing"}),
            inspect_models=lambda b: [],
            record_sidecar_pid=lambda *a: None,
            record_last_verb=lambda *a: None,
    ...
    if errors:
        ...
        return 1
    print("PASS selftest \u00b7 Attach Claude FAIL+next \u00b7 never silent stub")
    return 0
...
if __name__ == "__main__":
    ...
    print("usage: pfy_attach_usable_221.py --prove [BASE] | --selftest", file=sys.stderr)
    raise SystemExit(2)
