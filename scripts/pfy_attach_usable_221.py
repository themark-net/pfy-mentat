#!/usr/bin/env python3
"""Attach Claude developer-usable session -- cite #221 (shim).

Body lives in ``pfylib/attach.py`` parameterised by ``data/harnesses.json``
``claude-code.attach`` (T-0121). Same ``--prove`` / ``--selftest`` surface and
the same ``open_enterable_claude_session`` / ``live_session_reach`` names the
board loads. Missing Claude is FAIL+next, never a silent stub.
"""
...
def open_enterable_claude_session(**deps):
