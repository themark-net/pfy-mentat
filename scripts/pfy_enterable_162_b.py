...
def _enrich_smoke(result, base, inspect_models, model=""):
    """Attach usability smoke (models + one completion). Cite #193."""
    try:
        smoke_path = Path(__file__).resolve().parent / "pfy_attach_smoke_193.py"
        spec = importlib.util.spec_from_file_location("pfy_attach_smoke_193", smoke_path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        return mod.enrich_attach_result(
            result, base_url=base, inspect_models=inspect_models, model=model or ""
        )
    ...
    *,
    ...
    if real and pid_alive(real):
        ...
        return _enrich_smoke({
            ...
            "error": "",
            "engine": engine,
        }, base, inspect_models, model=model or "")
    clear_session_reach(STATE)
    try:
        if attach_base_path.is_file():
            attach_base_path.unlink()
