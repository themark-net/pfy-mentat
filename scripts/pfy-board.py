...
POST /eval runs ./pfy eval against the live local endpoint.
"""
...
def test_model():
    # Existing ./pfy eval against live local endpoint. Honest skip if no live engine.
    det = detector_json()
    ...
    if not PFY.is_file():
        return {"ok": False, "live": "FAIL", "copy": "FAIL eval", "error": "scripts/pfy missing"}
    rc, out = _run(["bash", str(PFY), "eval"], timeout=180.0)
    blob = (out or "")
    if rc != 0:
        return {
            "ok": False, "live": "FAIL", "copy": "FAIL eval",
            "error": (blob or "eval failed")[-400:],
            "stdout": blob[-800:],
        }
    return {"ok": True, "live": "PASS", "copy": "PASS eval", "stdout": blob[-800:]}
