#!/usr/bin/env python3
"""Attach Grok developer-usable session -- cite #202.

Mirrors Hermes attach-usable (#196) / OpenCode (#193): FreeToken-first
re-probe, child LOCAL_OPENAI_BASE_URL / OPENAI_BASE_URL, models list +
one smoke before ok:True / READY paint. Failures honest FAIL+next,
usable:false. start_monitor_sidecar stays for a separate monitor role.
"""
...
def _load_162_a():
    ...
    spec = importlib.util.spec_from_file_location("pfy_enterable_162_a_202", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod
...
def _apply_opencontext_env(env):
    """Handoff OpenContext store env into Attach Grok child. Cite #205."""
    path = Path(__file__).resolve().parent / "pfy_opencontext_205.py"
    if not path.is_file():
        return env
    spec = importlib.util.spec_from_file_location("pfy_opencontext_205_202", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
...
def _apply_attach_mode_env(env, STATE=None):
    """Handoff attach mode into Attach Grok child. Cite #208."""
    path = Path(__file__).resolve().parent / "pfy_attach_mode_208.py"
    if not path.is_file():
        return env
    spec = importlib.util.spec_from_file_location("pfy_attach_mode_208_202", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
...
def live_session_reach(STATE, pid_alive=None) -> str:
    """Return Grok session reach only if enterable pid is still alive. Cite #202."""
...
def open_enterable_grok_session(
    *,
    ...
    """Attach Grok + open/focus enterable terminal. Cite #202.
    ...
    against the live detect base AND models list + one smoke succeed (#202).
    Child env inherits LOCAL_OPENAI_BASE_URL / OPENAI_BASE_URL.
    Does not use start_monitor_sidecar (monitor remains a separate role).
    """
    ROOT = Path(ROOT)
    STATE = Path(STATE)
    hid = "grok"
    stub = stub_line or "./pfy start grok"
    ...
    if real and pid_alive(real) and prev_base == base:
        ...
        return {
            ...
            "copy": "attached grok pid %s \u00b7 %s \u00b7 %s" % (real, SESSION_REACH_OK, engine),
            "error": "",
            "base_url": base,
            "engine": engine,
            "model": model,
    ...
    if err:
        err = str(err).replace("OpenCode", "Grok")
...
def prove_cli(base):
    """CLI prove: models list + one smoke. Exit 0 only when usable. Cite #202."""
    next_step = "Launch env or ./pfy up"
...
if __name__ == "__main__":
    ...
    print("usage: pfy_attach_usable_202.py --prove [BASE]", file=sys.stderr)
    raise SystemExit(2)
