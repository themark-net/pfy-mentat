#!/usr/bin/env python3
"""Attach Codex developer-usable session -- cite #220.

Mirrors Grok attach-usable (#202) / Hermes (#196) / OpenCode (#193):
...
FAIL+next, usable:false. Missing Codex is FAIL+next, never a silent stub.
...
def _load_162_a():
    ...
    spec = importlib.util.spec_from_file_location("pfy_enterable_162_a_220", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod
...
def _apply_opencontext_env(env):
    """Handoff OpenContext store env into Attach Codex child. Cite #205."""
    path = Path(__file__).resolve().parent / "pfy_opencontext_205.py"
    if not path.is_file():
        return env
    spec = importlib.util.spec_from_file_location("pfy_opencontext_205_220", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
...
def _apply_attach_mode_env(env, STATE=None):
    """Handoff attach mode into Attach Codex child. Cite #208."""
    path = Path(__file__).resolve().parent / "pfy_attach_mode_208.py"
    if not path.is_file():
        return env
    spec = importlib.util.spec_from_file_location("pfy_attach_mode_208_220", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
...
def _codex_bin(which_bin):
    found = which_bin("codex") if callable(which_bin) else ""
    if found:
        return found
    home = Path.home() / ".local" / "bin" / "codex"
    try:
        if home.is_file() and os.access(str(home), os.X_OK):
            return str(home)
    except OSError:
        pass
    return ""
...
def live_session_reach(STATE, pid_alive=None) -> str:
    """Return Codex session reach only if enterable pid is still alive. Cite #220."""
...
def open_enterable_codex_session(
    *,
    ...
    """Attach Codex + open/focus enterable terminal. Cite #220.
    ...
    against the live detect base AND models list + one smoke succeed (#220).
    Child env inherits LOCAL_OPENAI_BASE_URL / OPENAI_BASE_URL.
    Missing Codex is FAIL+next (never a silent stub success).
    """
    ROOT = Path(ROOT)
    STATE = Path(STATE)
    hid = "codex"
    stub = stub_line or "./pfy start codex"
    bin_path = _codex_bin(which_bin)
    if not bin_path:
        clear_session_reach(STATE)
        return {
            "ok": False,
    ...
    if real and pid_alive(real) and prev_base == base:
        ...
        if active_harness_setter:
            active_harness_setter("codex")
        record_last_verb("start codex")
        return {
            "ok": True,
            "id": hid,
            "live": "READY",
            ...
            "copy": "attached codex pid %s \u00b7 %s \u00b7 %s" % (real, SESSION_REACH_OK, engine),
            "error": "",
            "base_url": base,
            "engine": engine,
            "model": model,
    ...
    if real and pid_alive(real):
        ...
        if active_harness_setter:
            ...
            try:
                (STATE / "active-harness").write_text("codex\n", encoding="utf-8")
            except OSError:
                pass
        record_last_verb("start codex")
        try:
...
def prove_cli(base):
    """CLI prove: models list + one smoke. Exit 0 only when usable. Cite #220."""
    next_step = "Launch env or ./pfy up"
...
def cmd_selftest():
    """Missing Codex and missing engine are FAIL+next, never ok:True. Cite #220."""
    import tempfile
    ...
    with tempfile.TemporaryDirectory(prefix="pfy-220-") as tmp:
        state = Path(tmp)
        missing = open_enterable_codex_session(
            ROOT=root,
            STATE=state,
            which_bin=lambda *a: "",
        ...
        check((missing.get("error") or "") == "codex missing", "missing error")
        check(NEXT_INSTALL in (missing.get("next_step") or ""), "missing next install")
        check(missing.get("live") == "FAIL", "missing live FAIL")

        noeng = open_enterable_codex_session(
            ROOT=root,
            STATE=state,
            which_bin=lambda *a: "/tmp/fake-codex-220",
            live_openai_base=lambda: ("", {"engine": "none", "status": "missing"}),
            inspect_models=lambda b: [],
            record_sidecar_pid=lambda *a: None,
            record_last_verb=lambda *a: None,
    ...
    if errors:
        ...
        return 1
    print("PASS selftest \u00b7 Attach Codex FAIL+next \u00b7 never silent stub")
    return 0
...
if __name__ == "__main__":
    ...
    print("usage: pfy_attach_usable_220.py --prove [BASE] | --selftest", file=sys.stderr)
    raise SystemExit(2)
